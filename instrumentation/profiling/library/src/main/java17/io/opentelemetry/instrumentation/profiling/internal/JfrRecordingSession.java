...
final class JfrRecordingSession implements AutoCloseable {
  ...
  private static void applyMemorySettings(Map<String, String> settings, ProfilingConfig config) {
    applyBooleanSetting(
        settings, "jdk.ObjectAllocationInNewTLAB#enabled", config, config.getMemoryAllocationSampling());
    applyBooleanSetting(
        settings,
    ...
    applyBooleanSetting(
        settings, "jdk.ObjectAllocationSample#enabled", config, config.getMemoryAllocationSampling());
    applyBooleanSetting(
